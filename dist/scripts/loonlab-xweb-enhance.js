 ...
 * @date 2026-06-04 10:15:00
 */

const $ = new Env("X(Twitter)网页版功能增强");
const PINNED_TIMELINES_ARGUMENT_KEY = "pinned_tags";
...
try {
  ...
  // "顶部标签页" - 优先从 Loon 插件 argument 读取配置动态生成 PinnedTimelines 数据
  if (req_url.includes("/PinnedTimelines?")) {
    ...
    const timelineIndex = buildTimelineIndex(allTimelines);

    // 优先读取 Loon 插件 argument，未提供时兼容历史 BoxJS 配置
    const selectedTagsStr = getPinnedTimelinesSelectionInput();
    let pinnedTimelines;

    if (selectedTagsStr && selectedTagsStr.trim()) {
      const selectedTags = splitPinnedTimelineTokens(selectedTagsStr);
      pinnedTimelines = [];
      const invalidTags = [];
      const addedTags = new Set();

      selectedTags.forEach(tag => {
        const timeline = resolveTimelineToken(tag, timelineIndex);
        if (!timeline) {
          invalidTags.push(tag);
          return;
        }

        if (addedTags.has(timeline.tag)) return;
        addedTags.add(timeline.tag);
        pinnedTimelines.push(timeline);
      });

      if (invalidTags.length > 0) {
        console.log(`⚠️未匹配到的标签输入: ${invalidTags.join(",")}`);
      }
      if (pinnedTimelines.length > 0) {
        console.log(`✅PinnedTimelines从argument/外部配置读取成功，共${pinnedTimelines.length}个标签`);
      } else {
        pinnedTimelines = [];
        console.log(`⚠️argument/外部配置未匹配到有效标签，返回空标签列表`);
      }
    } else {
      // 未配置时返回空标签列表
      pinnedTimelines = [];
      console.log(`✅PinnedTimelines未提供argument/外部配置，返回空标签列表`);
    }
...
function getPinnedTimelinesSelectionInput() {
  if (typeof $argument === "object" && $argument !== null) {
    const argumentValue = $argument[PINNED_TIMELINES_ARGUMENT_KEY];
    if (typeof argumentValue === "string" && argumentValue.trim()) {
      return argumentValue;
    }
  }

  if (typeof $argument === "string" && $argument.trim()) {
    const parsedValue = parseArgumentString($argument, PINNED_TIMELINES_ARGUMENT_KEY);
    if (parsedValue) {
      return parsedValue;
    }
    return $argument;
  ...
  return $.getdata("X_web_pinned_timelines") || "";
}

function parseArgumentString(argumentText, keyName) {
  const trimmedArgument = String(argumentText || "").trim();
  if (!trimmedArgument) return "";

  const kvMatch = trimmedArgument.match(new RegExp(`(?:^|[&;\\n])${escapeRegExp(keyName)}=([^&;\\n]+)`));
  if (kvMatch && kvMatch[1]) {
    return decodeArgumentValue(kvMatch[1]);
  }

  const jsonCandidate = $.toObj(trimmedArgument, null);
  if (jsonCandidate && typeof jsonCandidate === "object") {
    const value = jsonCandidate[keyName];
...
function decodeArgumentValue(value) {
  try {
    ...
    console.log(`⚠️argument 解码失败，使用原始值: ${error.message}`);
    return String(value);
  }
}

function splitPinnedTimelineTokens(value) {
  return String(value || "")
    .split(/[\n,，|]+/)
    .map(item => item.trim())
    .filter(Boolean);
...
function buildTimelineIndex(allTimelines) {
  const createBucket = () => Object.create(null);
  const timelineIndex = {
    tab_label: createBucket(),
    name: createBucket(),
    tag: createBucket(),
    scribe: createBucket()
  };
  ...
  allTimelines.forEach(timeline => {
    indexTimeline(timelineIndex.tab_label, timeline.tab_label, timeline);
    indexTimeline(timelineIndex.name, timeline.name, timeline);
    indexTimeline(timelineIndex.tag, timeline.tag, timeline);
    indexTimeline(timelineIndex.scribe, timeline.scribe, timeline);
  });

  return timelineIndex;
}

function indexTimeline(bucket, key, timeline) {
  const normalizedKey = normalizeTimelineToken(key);
  if (!normalizedKey) return;
  if (!bucket[normalizedKey]) {
    bucket[normalizedKey] = [];
  }
  bucket[normalizedKey].push(timeline);
}

function resolveTimelineToken(token, timelineIndex) {
  const normalizedToken = normalizeTimelineToken(token);
  if (!normalizedToken) return null;

  const matchers = [
    timelineIndex.tab_label[normalizedToken],
    timelineIndex.name[normalizedToken],
    timelineIndex.tag[normalizedToken],
    timelineIndex.scribe[normalizedToken]
  ];
  ...
  for (const matches of matchers) {
    ...
    if (matches.length > 1) {
      console.log(`⚠️标签输入 "${token}" 存在多个匹配，默认使用第一个: ${matches[0].name}`);
      return matches[0];
    }
  }
