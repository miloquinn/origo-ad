...
def fetch_text(url: str, cache_path: Path, use_cache: bool) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "egern-battery-adblock/1.0"})
...
def build_module(
    ...
    lines = [
        ...
        "#!author=Generated by egern-battery-adblock",
...
modules:
- name: Origo Battery AdBlock Lite
  url: {lite_url}
  enabled: true
- name: Origo Battery AdBlock Balanced
  url: {balanced_url}
  enabled: false
...
def main() -> int:
    ...
    balanced_module = build_module(
        "Origo Battery AdBlock Balanced",
        source_records,
        balanced_sections,
    ...
    lite_path = DIST_DIR / "origo-adblock-lite.module"
    balanced_path = DIST_DIR / "origo-adblock-balanced.module"
    lite_path.write_text(lite_module, encoding="utf-8")
    balanced_path.write_text(balanced_module, encoding="utf-8")

    base = args.base_url.rstrip("/")
    snippet = make_snippet(f"{base}/origo-adblock-lite.module", f"{base}/origo-adblock-balanced.module")
